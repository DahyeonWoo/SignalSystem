...
y1=3*(exp(40-10*t)-1).*(t>4)-3*(exp(10-10*t)-1).*(t>1);        % 라플라스 변환에 의한 출력 생성
% y = 3*heaviside(t - 4)*(exp(40 - 10*t) - 1) - 3*heaviside(t - 1)*(exp(10 - 10*t) - 1)
x=3*(t>1)-3*(t>4);                                             % 입력 신호 생성
h=10*exp(-10*t).*stepfun(t,0);                                 % 임펄스 응답 생성
